...
build_start = content.find(start_marker)
...
if build_start != -1 and build_end != -1:
    new_build_method = """  Widget _buildExtendBtn(String label, int days) {
    return Container(
  ...
  Widget build(BuildContext context) {
    ...
    final headerSection = [
      ...
      if (shop.validUntil != null) Text("Valid Until: ${fmt.format(shop.validUntil!)}", style: const TextStyle(color: Color(0xFF64748B), fontWeight: FontWeight.w600)),
        ...
        child: ElevatedButton.icon(
          ...
          onPressed: () async {
            ...
            try {
              ...
              if (mounted) NotificationHelper.showCenter(context, "Global Inventory injected into \\"${widget.shop.shopName}\\" successfully!", isError: false);
            } catch (e) {
              if (mounted) NotificationHelper.showCenter(context, "Failed to inject global inventory: $e", isError: true);
            } finally {
...
"""
    new_content = content[:build_start] + new_build_method
    with open("lib/presentation/master_admin/master_admin_screen.dart", "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Successfully replaced build method.")
else:
