...
class DesktopDashboardShell extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopDashboardShell> createState() =>
      _DesktopDashboardShellState();
}
...
class _DesktopDashboardShellState extends ConsumerState<DesktopDashboardShell> {
  ...
  Widget build(BuildContext context) {
    ...
    final isImpersonating = session?.id == 'host_admin'
        ? false
    ...
    return Scaffold(
      ...
      body: Row(
        children: [
          ...
          // ----------------------------------------------------
          _buildSidebar(settings, session, language, isImpersonating),
        ...
        children: [
          ...
          Container(
            padding: EdgeInsets.only(
              top: 40,
              bottom: 24,
              left: _isSidebarCollapsed ? 0 : 24,
              right: _isSidebarCollapsed ? 0 : 12,
            ),
            child: _isSidebarCollapsed
                ? Column(
                    children: [
                      _buildSidebarLogo(settings),
                ...
                if (isImpersonating) {
                  showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      content: const Text(
                        'Are you sure you want to exit impersonation and return to Customer Support?',
                      ),
                      actions: [
                        TextButton(
  ...
  Widget _buildCategoryHeader(String title) {
    if (_isSidebarCollapsed) return const SizedBox(height: 10);
    return Padding(
    ...
    return Container(
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: isActive ? iconAccentColor : Colors.transparent,
            borderRadius: BorderRadius.circular(10),
            boxShadow: isActive
                ? [
                    BoxShadow(
              ...
              Container(
                ...
                child: Center(
                  child: Icon(
                    ...
                    color: isActive
                        ? Colors.white
                        : iconAccentColor.withOpacity(0.7),
                    size: 16,
                  ),
                ),
              ),
  ...
  Widget _buildSidebarLogo(SettingsState settings) {
    return Container(
  ...
  Widget _buildContentArea() {
    switch (_currentRoute) {
      ...
      case 'support_ticket':
        final settings = ref.watch(settingsProvider);
        final shopCode =
            Hive.box<String>('settings').get('shopCode') ?? 'Unknown';
                      ...
                      NeumorphicButton(
                        ...
                        onTap: () async {
                          try {
                            ...
                            final shopName =
                                Hive.box<String>('settings').get('shopName') ??
                                'Shop';
                            ...
                            if (path != null) {
                              NotificationHelper.showCenter(
                                context,
                                'Backup successfully exported to:\n$path',
                                isError: false,
                              );
                            } else {
                              NotificationHelper.showCenter(
                            ...
                            if (context.mounted) {
                              NotificationHelper.showCenter(
                                context,
                                'Permission error. Failed to export database.',
                                isError: true,
                              );
                            }
                          }
                          ...
                          try {
                            ...
                            if (result != null &&
                              ...
                              if (success) {
                                ...
                                NotificationHelper.showCenter(
                                  context,
                                  'Failed to restore. Invalid or tampered backup file.',
                                  isError: true,
                                );
                              }
                            }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (printer.connectedDevice == null) {
      alerts.add({
        ...
        'desc':
            'No thermal printer is currently configured. Print receipts may fail.',
        'type': 'warning',
        'time': 'System Status',
    ...
    for (final p in outOfStock) {
      alerts.add({
        ...
        'desc':
            '${p.name} is completely out of stock! Please restock immediately.',
        'type': 'alert',
    ...
    for (final p in lowStock) {
      alerts.add({
        ...
        'desc':
            '${p.name} has only ${p.stockCount} items remaining in inventory.',
        'type': 'warning',
    ...
    final todayOrders = orders
        .where((o) => DateFormat('yyyy-MM-dd').format(o.date) == todayStr)
        .toList();
    for (final o in todayOrders) {
      alerts.add({
        'title': o.isVoided ? 'Sale Voided' : 'Sale Completed',
        'desc':
            'Order #${o.displayId} processed by ${o.staffName.isEmpty ? 'Admin' : o.staffName} - ₹${o.total.toStringAsFixed(2)}',
    ...
    final todayRefunds = refunds
        .where((r) => DateFormat('yyyy-MM-dd').format(r.date) == todayStr)
        .toList();
    for (final r in todayRefunds) {
      alerts.add({
        'title': 'Refund Processed',
        'desc':
            'Ref: ${r.originalOrderId} refunded ₹${r.amountRefunded.toStringAsFixed(2)} - ${r.reason}',
        'type': 'info',
